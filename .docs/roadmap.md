...
| 2026-09-20 | HB-37 / HB-38 完成 | 发版与提交流程去冗余，起因是 Hubery 发现「检查跑了三遍」。**HB-37**：`release` 脚本加 `bumpp --no-verify`——bumpp 的 `--verify` 默认 `true`，源码证实 `--no-verify` 只加在 `git commit` 上（`gitPush` 是裸 `git push`，拦不住 pre-push，这也是「把检查移到 pre-push」方案被否掉的原因）。**HB-38**：`lint-staged.config.mjs` 收窄为 `'*': 'eslint --fix'`，`ci.yml` **同时**加 push 触发（main/dev）——两者必须配对，CI 此前只在 PR 触发，而活儿要在 dev 上攒好几个提交才开 PR，光收窄 hook 会让那一整段无人把关。依据是 antfu/eslint-config、unocss、vueuse 三家的 pre-commit 一律只跑 `eslint --fix`。验证：全套门禁 241 单测 + 57 E2E 全绿；实跑 `npx lint-staged` 确认唯一任务是 `eslint --fix`；**变异实证**——植入 `const wrong: number = 'not a number'` 后 lint-staged 全绿而 `tsc` 报 TS2322，证明类型检查真从 hook 里去掉了、且变异本身有效；推送后 **本仓库第一次出现 `push` 事件触发的 CI run 并 success**。⚠️ 两条叠加后 HB-37 的收益从 11.6s 缩到 1s 以内（pre-commit 已经只剩 eslint），保留它是因为对着一个版本号跑 eslint 仍是零价值操作。⚠️ 已知代价：PR 期间 CI 会双跑（push 一次、pull_request 一次），现有 `concurrency` 用 `github.ref`，两者 ref 不同拦不住 |
